...
//	retval = usb_bulk_msg(device,usb_rcvbulkpipe(device,BULK_EP_IN),bulk_buf_read,1024,&read_cnt,5000);
	retval = usb_interrupt_msg(dev->udev,usb_rcvintpipe(device,0x81),bulk_buf_read,1024,&read_cnt,5000);
	if(retval < 0)
	{
...
//	retval = usb_bulk_msg(device,usb_sndbulkpipe(device,BULK_EP_OUT),bulk_buf,1024,&wrote_cnt,5000);
	retval = usb_control_msg(dev->udev,usb_sndctrlpipe(device,0),0x00,0x20,0,0,bulk_buf,wrote_cnt,5000);
	mutex_unlock(&dev->io_mutex);
	if(retval < 0)
	{
...
int open_dev(struct inode *inode, struct file *open_file) 
{
	...
	interface = usb_find_inetrface(&bt_driver,subminor);
	if(!interface)
	{
